...
# License: MIT and idk what licence does openand immoralwrt use
#its hard to write when you are drunk
...
if (whiptail --backtitle "Proxmox VE Helper Scripts" --title "ImmoralWrt VM" --yesno "This will create a New OpenWrt VM. Proceed?" 10 58); then
  :
else
...
function advanced_settings() {
  ...
  if (whiptail --backtitle "Proxmox VE Helper Scripts" --title "ADVANCED SETTINGS COMPLETE" --yesno "Ready to create ImmoralWrt VM?" --no-button Do-Over 10 58); then
...
if [ -z "$VALID" ]; then
  ...
  while [ -z "${STORAGE:+x}" ]; do
    STORAGE=$(whiptail --backtitle "Proxmox VE Helper Scripts" --title "Storage Pools" --radiolist \
      "Which storage pool you would like to use for the ImmoralWrt VM?\n\n" \
...
URL="https://downloads.immortalwrt.org/releases/$stableversion/targets/x86/64/openwrt-$stableversion-x86-64-generic-ext4-combined.img.gz"
...
qm start $VMID
sleep 15
msg_ok "Network interfaces are being configured as ImmoralWrt initiates."
send_line_to_vm ""
...
if [ "$START_VM" == "yes" ]; then
  msg_info "Starting ImmoralWrt VM"
  qm start $VMID
  msg_ok "Started ImmoralWrt VM"
fi
VLAN_FINISH=""
