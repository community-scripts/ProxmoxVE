...
function MobileSidebar() {
  ...
  const [lastViewedScript, setLastViewedScript] = useState<Script | undefined>(
    undefined
  );
  ...
  const [tempSelectedScript, setTempSelectedScript] = useState<string | null>(
    null
  );
  ...
  const currentSelectedScript = isOnScriptsPage
    ? selectedScript
    : tempSelectedScript;
  const currentSelectedCategory = isOnScriptsPage
    ? selectedCategory
  ...
  return (
    <Sheet open={isOpen} onOpenChange={handleOpenChange}>
      ...
      <SheetHeader className="sr-only border-border border-b px-6 pt-2 pb-4">
        <SheetTitle className="sr-only">Categories</SheetTitle>
      </SheetHeader>
      <SheetContent
        side="left"
        className="flex w-full max-w-xs flex-col gap-4 overflow-hidden px-0 pb-6"
      >
        <div className="flex h-full flex-col gap-4 overflow-y-auto">
          {isLoading && !hasLinks ? (
            <div className="flex w-full flex-col items-center justify-center gap-2 px-6 py-4 text-muted-foreground text-sm">
              Loading categories...
            </div>
          ) : (
            <div className="flex flex-col gap-4 px-4">
              <Sidebar
                ...
                setSelectedScript={
                  isOnScriptsPage ? setSelectedScript : setTempSelectedScript
                }
          ...
          {currentSelectedScript && lastViewedScript ? (
            <div className="flex flex-col gap-3 px-4">
              <p className="font-medium text-sm">Last Viewed</p>
              <ScriptItem
