...
# Get VM IPs using simple methods
get_vm_ips() {
    local vmid=$1 ips=""
    ...
    for mac in $mac_addresses; do
        ...
        if [[ -n "$ip" && "$ip" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ ]]; then
            debug_log "vm $vmid: found IP $ip via ARP for MAC $mac_lower"
            ips+="$ip "
        fi
    done
    
    # Method 2: QM guest agent if ARP failed
    if [[ -z "$ips" ]] && command -v qm >/dev/null 2>&1; then
        local qm_ips=$(timeout 5 qm guest cmd "$vmid" network-get-interfaces 2>/dev/null | grep -oE '([0-9]{1,3}\.){3}[0-9]{1,3}' | grep -v "127.0.0.1" | head -2)
    ...
    # Method 3: DHCP leases if still no IP
    if [[ -z "$ips" ]]; then
        for mac in $mac_addresses; do
            local mac_lower=$(echo "$mac" | tr '[:upper:]' '[:lower:]')
            for dhcp_file in "/var/lib/dhcp/dhcpd.leases" "/var/lib/dhcpcd5/dhcpcd.leases"; do
                if [[ -f "$dhcp_file" ]]; then
                    local dhcp_ip=$(grep -A 10 "ethernet $mac_lower" "$dhcp_file" 2>/dev/null | grep "binding state active" -A 5 | grep -oE "([0-9]{1,3}\.){3}[0-9]{1,3}" | head -1)
