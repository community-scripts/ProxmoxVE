...
whiptail --backtitle "Incus Scripts" --title "All Templates" --yesno "This will allow for the creation of one of the many Template LXC Containers. Proceed?" 10 68 || exit
TEMPLATE_MENU=()
MSG_MAX_LENGTH=0
...
TEMPLATE=$(whiptail --backtitle "Incus Scripts" --title "All Template LXCs" --radiolist "\nSelect a Template LXC to create:\n" 16 $((MSG_MAX_LENGTH + 58)) 10 "${TEMPLATE_MENU[@]}" 3>&1 1>&2 2>&3 | tr -d '"') || exit
[ -z "$TEMPLATE" ] && {
  whiptail --backtitle "Incus Scripts" --title "No Template LXC Selected" --msgbox "It appears that no Template LXC container was selected" 10 68
  msg "Done"
  exit
}
  ...
  *) false || die "Invalid storage class." ;;
  ...
  if [ $((${#MENU[@]} / 3)) -eq 0 ]; then
    ...
    while [ -z "${STORAGE:+x}" ]; do
      STORAGE=$(whiptail --backtitle "Incus Scripts" --title "Storage Pools" --radiolist \
